...
# preflights would hard-gate immediately, which we don't want yet. The framed
# document PDFs (schematic, assembly, fabrication drawing) are generated by
# scripts/gen_docs.sh with kicad-cli; KiBot generates only the interactive HTML
# BOM. dev and release share both, so they render identically. Output lands in
# reports/.
#
