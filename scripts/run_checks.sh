...
# --- Artifacts (never gate; generated by KiBot) -------------------------------
# Skip preflights (--skip-pre all): ERC/DRC gating is handled above under the
# ENFORCE toggle. KiBot starts its own virtual display (xvfbwrapper) for the
# outputs that need one (pcb_print frame plotting), so we call it directly —
# no xvfb-run wrapper (the image ships no xauth).
kibot -c usb3_fiber.kibot.yaml -e "$SCH" -b "$PCB" \
  -d reports --skip-pre all \
  schematic_pdf assembly_top assembly_bottom ibom \
